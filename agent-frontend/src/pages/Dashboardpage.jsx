...
const KPI_TEMPLATE = [
  ...
  { key: "alerts",   label: "Alertes",      icon: IconShield,   color: AWB.danger,  base: 9,    variance: 4  },
];
...
export default function DashboardPage() {
  ...
  // Les alertes AML sont sensibles : visibles uniquement par l'Admin et
  // le Back Office, jamais par un agent Front Office.
  const canSeeAlerts =
    profile?.primaryRole === ROLES.ADMIN ||
    profile?.primaryRole === ROLES.BACK_OFFICE;
  ...
  const kpis = useMemo(() => {
    const template = canSeeAlerts
      ? KPI_TEMPLATE
      : KPI_TEMPLATE.filter((k) => k.key !== "alerts");
  ...
  return (
    <div className="dash-root">
      ...
      {/* ── KPI row ── */}
      <div
        className="dash-grid dash-row-kpi"
        style={{
          marginBottom: 16,
          // La grille .dash-row-kpi est figée à 4 colonnes en CSS. Quand le
          // Front Office n'a que 3 KPI (alertes masquées), on rebascule sur
          // 3 colonnes pour que les cartes occupent toute la largeur sans
          // laisser une 4ᵉ colonne vide à droite.
      ...
      {/* ── Dossiers + alerts ── */}
      <div
        className="dash-grid dash-row-bottom"
        style={!canSeeAlerts ? { gridTemplateColumns: "1fr" } : undefined}
      >
        <div className="card">
          <div className="card-title" style={{ justifyContent: "space-between", marginBottom: 12 }}>
            <span style={{ display: "flex", alignItems: "center", gap: 10 }}>
