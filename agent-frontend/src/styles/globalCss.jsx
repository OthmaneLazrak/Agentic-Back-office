...
@import url('https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700&family=Playfair+Display:wght@600;700&display=swap');
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .sidebar-history-refresh:hover { color: rgba(255,255,255,0.85); }
  .sidebar-history-refresh.spin { animation: hist-spin 0.8s linear infinite; }
  ...
  .sidebar-history-list::-webkit-scrollbar { width: 5px; }
  .sidebar-history-list::-webkit-scrollbar-thumb { background: rgba(255,255,255,0.12); border-radius: 3px; }
  ...
  .hist-item:hover { background: rgba(255,255,255,0.07); border-color: rgba(255,255,255,0.12); }
  .hist-kind {
    font-size: 9px; font-weight: 700; letter-spacing: 0.5px;
    padding: 2px 5px; border-radius: 5px; flex-shrink: 0;
  }
  .hist-kind.kyc    { background: rgba(245,168,0,0.16);  color: ${AWB.gold}; }
  .hist-kind.cheque { background: rgba(59,130,246,0.18); color: #93C5FD; }
  .hist-main { flex: 1; min-width: 0; }
  .hist-title {
    font-size: 11.5px; color: rgba(255,255,255,0.85);
    white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
  }
  .hist-meta { font-size: 9.5px; color: rgba(255,255,255,0.4); margin-top: 1px; }
  .hist-status {
    font-size: 8.5px; font-weight: 700; padding: 2px 6px; border-radius: 6px;
    flex-shrink: 0; text-transform: uppercase; letter-spacing: 0.3px;
  }
  .hist-status.pending   { background: ${AWB.warningSoft}; color: ${AWB.warning}; }
  .hist-status.approved  { background: ${AWB.successSoft}; color: ${AWB.success}; }
  .hist-status.rejected  { background: ${AWB.dangerSoft};  color: ${AWB.danger}; }
  .hist-status.escalated { background: ${AWB.infoSoft};    color: ${AWB.info}; }
  .hist-empty { font-size: 11px; color: rgba(255,255,255,0.35); padding: 6px 20px; line-height: 1.5; }

  /* ─────────────────────── History page filter ─────────────────────── */
  .filter-wrap { position: relative; }
  .filter-btn { display: inline-flex; align-items: center; gap: 7px; }
  .filter-btn.active { border-color: ${AWB.navy}; color: ${AWB.navy}; }
  .filter-count {
    background: ${AWB.gold}; color: ${AWB.navy};
  ...
  .filter-panel {
    ...
    background: white; border: 1px solid ${AWB.slate200};
    border-radius: 11px; box-shadow: 0 14px 40px rgba(13,27,42,0.16);
    width: 220px; padding: 8px;
    display: flex; flex-direction: column;
    animation: modal-fade 0.12s ease;
  }
  .filter-group { display: flex; flex-direction: column; padding: 4px 0; }
  .filter-group + .filter-group { border-top: 1px solid ${AWB.slate100}; margin-top: 4px; }
  .filter-group-label {
    font-size: 10px; font-weight: 700; letter-spacing: 0.6px; text-transform: uppercase;
    color: ${AWB.slate400}; padding: 4px 8px 6px;
  }
  .filter-option {
    display: flex; align-items: center; justify-content: space-between;
    width: 100%; text-align: left;
    padding: 8px 8px; border: none; background: none; cursor: pointer;
    font-size: 13px; color: ${AWB.slate700}; border-radius: 7px;
    font-family: inherit;
  }
  .filter-option:hover { background: ${AWB.slate50}; }
  .filter-option.active { background: ${AWB.slate50}; color: ${AWB.navy}; font-weight: 600; }
  .filter-foot { border-top: 1px solid ${AWB.slate100}; margin-top: 4px; padding-top: 6px; }
  .filter-reset {
    width: 100%; text-align: center;
    padding: 7px; border: none; background: none; cursor: pointer;
    font-size: 12px; font-weight: 500; color: ${AWB.danger}; border-radius: 7px;
    font-family: inherit;
  }
  .filter-reset:hover:not(:disabled) { background: ${AWB.dangerSoft}; }
  .filter-reset:disabled { color: ${AWB.slate300}; cursor: not-allowed; }

  /* ─────────────────────── History page root (scrollable) ─────────────────────── */
  /* Bloc simple (PAS flex) : la carte garde toute sa hauteur de contenu
     — fond blanc + pagination inclus — et c'est ce conteneur qui défile. */
  .hist-root {
    height: 100%;
    overflow-y: auto;
    padding-right: 4px;       /* petite marge pour la scrollbar */
  }
  /* La liste ne crée pas son propre scroll ici : c'est la page qui défile. */
  .hist-root .bo-list { overflow: visible; }
  ...
  .page-btn, .page-nav {
    ...
    border: 1px solid ${AWB.slate200}; background: white; border-radius: 8px;
    font-size: 13px; color: ${AWB.slate600}; cursor: pointer;
    display: inline-flex; align-items: center; justify-content: center;
    font-family: inherit; transition: all 0.15s ease;
  }
  .page-btn:hover:not(.active), .page-nav:hover:not(:disabled) {
    background: ${AWB.slate50}; border-color: ${AWB.slate300}; color: ${AWB.navy};
  }
  .page-btn.active { background: ${AWB.navy}; border-color: ${AWB.navy}; color: white; font-weight: 600; }
  .page-nav:disabled { opacity: 0.45; cursor: not-allowed; }
  .page-ellipsis { padding: 0 2px; color: ${AWB.slate400}; font-size: 13px; align-self: flex-end; }
  ...
  .hist-search:focus-within { border-color: ${AWB.slate400}; }
  .hist-search-input {
    border: none; outline: none; background: transparent;
    font-size: 13px; color: ${AWB.slate900}; width: 100%;
    font-family: inherit;
  }
  ...
  .modal-head {
    ...
    padding: 14px 18px; border-bottom: 1px solid ${AWB.slate200};
    flex-shrink: 0;
  }
  .modal-kind {
  ...
  .modal-kind.kyc    { background: ${AWB.goldLight}; color: ${AWB.goldDark}; }
  .modal-kind.cheque { background: ${AWB.infoSoft};  color: ${AWB.info}; }
  .modal-title {
    font-size: 14px; font-weight: 600; color: ${AWB.navy};
    flex: 1; min-width: 0;
    white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
  }
  .modal-close {
    background: none; border: none; cursor: pointer;
    color: ${AWB.slate500}; padding: 5px; border-radius: 7px;
    display: flex; align-items: center;
    transition: all 0.15s ease;
  }
  .modal-close:hover { background: ${AWB.slate100}; color: ${AWB.navy}; }
  .modal-body { padding: 18px 20px; overflow-y: auto; }
  .modal-docs { display: flex; gap: 12px; flex-wrap: wrap; margin-bottom: 4px; }
  .modal-docs .bo-doc { flex: 1 1 200px; min-width: 180px; }
  ...
  /* Split-pane vérification manuelle : documents (gauche) · analyse IA (droite) */
  .bo-verify-grid {
    display: grid;
    grid-template-columns: 1fr 1fr;
    gap: 16px;
  ...
  @media (max-width: 1100px) {
    .bo-verify-grid { grid-template-columns: 1fr; }
  }
