...
const PER_USER_STORAGE_KEYS = ["awb.kyc.state.v1", "awb.cheque.state.v1"];
...
export function AuthProvider({ children }) {
  ...
  // The Keycloak client `agent-frontend` must have "Direct Access Grants Enabled"
  ...
  const loginWithPassword = useCallback(async (username, password, { remember = true } = {}) => {
    ...
    // Après une connexion réussie, on atterrit toujours sur le Dashboard
    // (le refresh de page, lui, restaure la dernière page consultée).
    try { localStorage.setItem("awb.activePage", "dashboard"); } catch { /* noop */ }
